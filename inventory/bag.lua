...
function gw_bg_loadCurrency()
    ...
    for i = 1, #curwin.buttons do
        ...
        if idx <= 0 or idx >= currencyCount then
            ...
            local name, isHeader, isExpanded, isUnused, isWatched, count, icon, maximum, hasLimit, curWeekly, _ =
                GetCurrencyListInfo(idx)
            if isHeader then
                slot.item:Hide()
                slot.item.CurrencyID = nil
                slot.item.CurrencyIdx = nil
