...
addChange("11.0.0", {
    {GW.Enum.ChangelogType.change, [=[Patch 12.1 (Retail): full compatibility update - all removed APIs replaced, the Social/Friends frame skin was rebuilt for Blizzards new SocialUI (all lists, sorted tabs with new tab icons, quick join notifications), and tooltips for the new secure auras use Blizzards spell id CVar so the "Advanced Tooltip ID Modifier" setting keeps working live without a reload]=]},
    {GW.Enum.ChangelogType.change, [=[Auras (Retail): player buff/debuff bars, target, focus, pet and party auras run on Blizzards new AuraContainer system now - the containers track and render the auras inside Blizzards secure environment, so everything keeps working while aura values are secret in combat; the GW2 look (black frames, dispel colors, cooldown swipe rings, big own auras) is unchanged. The other game versions keep the proven old engine untouched.]=]},
    {GW.Enum.ChangelogType.feature, [=[Auras: new "Ignored Auras" setting - enter a spell id and the aura is never shown on that frame; the list shows icon, name and tooltip per entry and lives on the respective settings page. Available for target, focus, pet and party frames on ALL game versions and for the player buff/debuff bars on Retail]=]},
    {GW.Enum.ChangelogType.feature, [=[Auras: new "Aura Sorting" setting - default, remaining time or name, each ascending or descending. Available for the player bars on all game versions (replaces the old sort method + sort direction pair, existing choices are migrated) and for target, focus and pet on Retail]=]},
    {GW.Enum.ChangelogType.feature, [=[Party (Retail): the "Dungeon & Raid Debuffs" list and its scale setting work on the party frames again]=]},
    {GW.Enum.ChangelogType.change, [=[Raid grids (Retail): buffs, debuffs and the aura indicators run on Blizzards AuraContainer system now and keep working while aura values are secret in combat. The indicator icons, stack counts, cooldown swipes and the duration bar are driven by the game engine itself. The "Dungeon & Raid Debuffs" emphasis works on the grids again (was disabled since 12.0), dispellable debuffs keep their own scale, and nothing renders twice]=]},
    {GW.Enum.ChangelogType.feature, [=[Raid grids (ALL game versions): the aura indicator dropdowns accept ANY spell now - pick "Custom Spell ID..." and enter the id manually; the predefined class spells stay available as before, custom spells show with a neutral white indicator color]=]},
    {GW.Enum.ChangelogType.feature, [=[Raid grids (ALL game versions): the "Ignored Auras" list lives per grid type now (party, raid 10/25/40, main tank, pets) as a spell id list with icon/name/tooltip on the respective settings page - it replaces the old global name based text field and starts pre-filled with the previously hardcoded ignore list; custom entries from the old text field have to be re-added once]=]},
    {GW.Enum.ChangelogType.change, [=[Raid grids: the private aura module was removed - the 12.1 API merged private auras into the normal (secret) debuffs, they show up in the regular debuff display now]=]},
    {GW.Enum.ChangelogType.feature, [=[Auras (Retail): the "Important" filter is back - 12.1 reintroduced Blizzards IMPORTANT aura flag, it is available again in the advanced buff/debuff filters of the raid grids, target, focus and pet frames. The raid grid debuff filters default to important debuffs (plus dispellable ones)]=]},
    {GW.Enum.ChangelogType.feature, [=[Auras (Retail): stealable buffs on target and focus get their highlight border back - the engine now decides this, so it also works while aura values are secret in combat]=]},
    {GW.Enum.ChangelogType.feature, [=[Auras (Retail): your own debuffs on target and focus show a glow during the pandemic window (the refresh window at the end of the duration) - driven by the game engine, no timers in the addon]=]},
    {GW.Enum.ChangelogType.feature, [=[Raid grids (Retail): debuffs YOU can dispel get a small dispel type icon in their corner]=]},
    {GW.Enum.ChangelogType.change, [=[Raid grids (Retail): the "show aura tooltips out of combat" mode is handled by the game engine now instead of toggling the mouse on every combat state change - more reliable, especially when entering combat]=]},
    {GW.Enum.ChangelogType.feature, [=[Class powers (Retail): the aura driven class power bars removed in 12.0 are back, powered by a new secret-proof tracker - protection paladin Shield of the Righteous, havoc demon hunter Metamorphosis, beast mastery hunter Barbed Shot (with countdown), survival hunter Mongoose Fury (stacks + countdown) arms warrior Rend on the target, fury warrior Enrage and protection warrior Shield Block (incl. Last Stand with the Bolster talent); the bars are driven by the game engine itself, so they keep running while combat values are secret]=]},
    {GW.Enum.ChangelogType.feature, [=[Dynamic HUD (Retail): the aura driven combat HUD backgrounds removed in 12.0 are back (Avenging Wrath, Metamorphosis, Ascendance, Dark Transformation, Dragonrage, Eclipse, ...) including the model effects - the art is toggled by the game engine itself, so it works while aura values are secret]=]},
    {GW.Enum.ChangelogType.change, [=[Profile export/import runs fully on Blizzards C_EncodingUtil now (CBOR serialization, deflate compression, base64 encoding) - the bundled LibDeflate, LibBase64, LibCompress and AceSerializer libraries were removed (a tiny built-in reader keeps legacy strings importable). New export strings start with "!GW3!", old "!GW2!" strings stay importable; the converter for the ancient pre-Deflate format was retired]=]},
    {GW.Enum.ChangelogType.bug, [=[Health globe (Retail): absorb shields and incoming heals sit on top of the current health fill again instead of filling from the bottom - the stacking is pure engine geometry (anchored to the health fill, revealed by a mask), so it works while health values are secret in combat]=]},
    {GW.Enum.ChangelogType.bug, [=[Dodge bar (Retail): several fixes - the bar was empty after a login or reload instead of full, abilities without charges (Disengage, Blink, ...) never showed their cooldown at all, and the fill now spans the whole visible arc so it starts moving right away and finishes exactly when the cooldown does]=]},
    {GW.Enum.ChangelogType.feature, [=[Dodge bar (ALL game versions): shows the remaining cooldown of the tracked ability as text - on Retail driven by the game engine, so it stays exact while cooldown values are secret in combat. Abilities with several charges show the available charges next to it, and their bar runs the cooldown of the charge that is currently recharging. Can be turned off in the player settings ("Show Dodge Bar Cooldown Text")]=]},
    {GW.Enum.ChangelogType.feature, [=[Pandemic highlight (Retail): your own auras on the player, target, focus, pet, party and raid frames - including the raid aura indicators - get a border glow while they are inside their pandemic window: the last 30% of their duration, where refreshing adds the remaining time on top. Driven by the game engine, so it works while aura durations are secret in combat. Can be turned off per frame in the respective frame settings ("Pandemic Highlight")]=]},
    {GW.Enum.ChangelogType.feature, [=[Dispel type icon (Retail): debuffs show their dispel type (magic, curse, poison, disease) as a small icon in their corner - previously this only existed on the raid frames, now it covers the player, target, focus, pet and party frames as well. Can be turned off per frame in the respective frame settings ("Show Dispel Type Icon")]=]},
    {GW.Enum.ChangelogType.bug, [=[Fixed errors on a reload during combat (damage meter skin, gossip frame keyboard handling)]=]},
    {GW.Enum.ChangelogType.bug, [=[Bank (Classic): the "buy bank slot" confirmation shows the price again - Blizzards inert bank frame no longer maintained the slot cost after the bag rework]=]},
    {GW.Enum.ChangelogType.bug, [=[Bank (Classic): items in the main bank slots show their tooltip again - the generic container tooltip returns nothing for the bank container there, the slots resolve to inventory slots now (like Blizzards bank does)]=]},
    {GW.Enum.ChangelogType.bug, [=[Game menu: fixed "AddOn tried to call the protected function Button:SetSize()" when opening the menu while in combat - the menu panel is protected there, the skipped button sizing is re-applied once combat ends]=]},
    {GW.Enum.ChangelogType.bug, [=[Action bars: hotkey labels no longer shift position or lose their short format when the game detects a gamepad (e.g. hybrid devices like the Razer Tartarus) - the GW position is re-applied after every binding update. Long bindings like "CTRL+!" no longer ellipsize, and the multi bar/pet/stance hotkeys use a larger, readable font now]=]},
