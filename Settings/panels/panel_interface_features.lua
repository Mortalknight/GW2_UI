...
local function LoadInterfaceFeaturesPanel(sWindow)
    ...
    p:AddOption(L["Missing enchant and socket warning"], L["Marks enchantable slots without an enchant and empty sockets in red."], {getterSetter = "windows.character.itemInfoMissing", callback = function() GW.ToggleCharacterItemInfo() end, groupHeaderName = L["UI Windows"], hidden = not (GW.Retail or GW.Forever), dependence = {["windows.character.enabled"] = true, ["windows.character.itemInfo"] = true}})
    ...
    p:AddOption(TRADE_SKILLS, L["Enable the profession replacement."], {getterSetter = "windows.profession.enabled", callback = function() GW.ShowRlPopup = true end, groupHeaderName = L["UI Windows"], hidden = not GW.isModern, isMasterToggle = true})
