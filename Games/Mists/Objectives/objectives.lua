    ...
    for _, quest in pairs(sorted) do
        if shouldShowHeader then
            ...
            if block and block:IsShown() and block.questID == quest.questId and block.gwSignature == signature then
                ...
                if block.questLogIndex ~= quest.questLogIndex then
                    -- feeds the quest item button and the click handlers, it shifts when
                    -- other quests come and go without our content changing
