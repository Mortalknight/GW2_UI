...
local function SkinSlots()
    for _, slot in pairs({InspectPaperDollItemsFrame:GetChildren()}) do
        -- the talents button is a child as well on retail; a plain return here used to abort the whole skin
...
local function SkinPvpFrame(frameWidth)
    ...
    local previous
    local index = 0
    for _, row in ipairs({ pvp.RatedBG, pvp.Arena2v2, pvp.Arena3v3, pvp.RatedSoloShuffle, pvp.RatedBGBlitz }) do
        if row then
            index = index + 1
            SkinPvpStatRow(row, previous, frameWidth - 40, index)
            previous = row
        end
    end
...
local function SkinInspectFrameOnLoad()
    ...
    -- the talents button hangs off the items frame on retail, off the paperdoll frame on camelot
    local talents = InspectPaperDollFrame.InspectTalents or InspectPaperDollItemsFrame.InspectTalents
    if talents then
        talents:GwSkinButton(false, true)
    end
