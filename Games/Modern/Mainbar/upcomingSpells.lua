...
local function CacheTrainerRewards()
    if scanningTrainer or not GetNumTrainerServices or C_Trainer.GetTrainerType() ~= Enum.TrainerType.General then
        return
    end
...
local function UpdateUpcomingSpells()
    ...
    for _, reward in ipairs(GetTrainerCache() or {}) do
        ...
        if not seenNames[key] and not (reward.spellID and seen[reward.spellID]) and not IsRewardKnown(reward) then
            ...
            local entry = {id = reward.spellID, name = reward.name, subText = reward.subText, icon = reward.icon, level = reward.level}
