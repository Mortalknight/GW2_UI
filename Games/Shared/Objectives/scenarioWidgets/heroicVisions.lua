...
local function addHeroicVisionsData(block)
    ...
    if mapIds[mapId] then -- Heroic Vision for OG and SW
        ...
        if info then
            block:AddObjective(GW.ParseCriteria(info.quantity, 0, info.name), { finished = false, objectiveType = "monster", qty = info.quantity, firstObjectivesYValue = -5 })
        end
    end
end
