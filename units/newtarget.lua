...
function gw_registerNewUnitFrame(unitToWatch, frameType)
   ...
   function target_OnEvent(self,event,unit)
        ...
        if event =='UNIT_MAXHEALTH' or event=='UNIT_HEALTH' and unit==unitToWatch then   
            updatHealthValues(thisName,unitToWatch)
        end        
        -- if power changed add the change to animation
        if event=='UNIT_MAXPOWER' or event=='UNIT_POWER_UPDATE' and unit==unitToWatch then
    ...
    targetF:RegisterEvent("UNIT_POWER_UPDATE");
...
function updateCastingbar(thisName,unitToWatch)
        ...
        local name, text, texture, startTime, endTime, _, castID, notInterruptible, spellID = UnitCastingInfo(self.unit)
        casting = false
        channel = false
            
         
        if name==nil then
            name, _, texture, startTime, endTime, _, notInterruptible = UnitChannelInfo(unitToWatch)
            spellType = 1;
        end
