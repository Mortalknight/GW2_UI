...
local function setUnitPortraitFrame(self)
    ...
    local txt
    local border = "normal"
    local showItemLevel = UnitIsPlayer(self.unit) and not InCombatLockdown() and CheckInteractDistance(self.unit, 4) and CanInspect(self.unit) and GW.settings[self.unit .. "_SHOW_ILVL"]
...
local function target_OnEvent(self, event, unit)
    ...
    if IsIn(event, "PLAYER_TARGET_CHANGED", "ZONE_CHANGED", "FORCE_UPDATE") then
        if event == "PLAYER_TARGET_CHANGED" and not InCombatLockdown() and UnitIsPlayer(self.unit) and  CheckInteractDistance(self.unit, 4) and CanInspect(self.unit) and GW.settings.target_SHOW_ILVL then
            local guid = UnitGUID(self.unit)
            if guid then
