...
function gw_update_raidframe_awayData(self)
    ...
    if self.targetmarker ~= nil and gwGetSetting('RAID_UNIT_MARKERS') ==true then 
...
function gw_raidframes_updateAuras(self)
    ...
    for i=1,40 do
        local name, icon, count, dispelType, duration, expires, caster, isStealable, nameplateShowPersonal, spellID, canApplyAura, isBossDebuff, _, nameplateShowAll, timeMod, value1, value2, value3 = UnitBuff(self.unit,i)
...
function gw_raidframes_updateDebuffs(self)
    ...
    for i=1,40 do
        ...
        DebuffLists[i] ={}
        DebuffLists[i]['name'],  DebuffLists[i]['icon'],  DebuffLists[i]['count'],  DebuffLists[i]['dispelType'],  DebuffLists[i]['duration'],  DebuffLists[i]['expires'],  DebuffLists[i]['caster'],  DebuffLists[i]['isStealable'],  DebuffLists[i]['shouldConsolidate'],      DebuffLists[i]['spellID']  =  UnitDebuff(self.unit,i,filter)
