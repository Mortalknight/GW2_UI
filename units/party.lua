    ...
    for i = 1, MAX_PARTY_MEMBERS + 1 do
        ...
        if i == 5 then
            if not GW.settings.PARTY_PLAYER_FRAME then
                ...
                frame.PetFrame:SetScript("OnEvent", nil)
            else
                frame:SetScript("OnEvent", frame.OnEvent)
                frame.PetFrame:SetScript("OnEvent", frame.OnEvent)
            end
        end
    end
...
local function CreatePartyFrame(i, isPlayer)
    ...
    local petFrame = CreateFrame("Button", "GwPartyPetFrame" .. i, UIParent, "GwPartyPetFrame")
    petFrame.unit = petUnit
    petFrame:SetAttribute("unit", petUnit)
    petFrame.isPet = true
    ...
    for _, ev in ipairs({ "UNIT_AURA", "UNIT_LEVEL", "UNIT_PHASE", "UNIT_HEALTH", "UNIT_MAXHEALTH", "UNIT_POWER_UPDATE", "UNIT_MAXPOWER", "UNIT_NAME_UPDATE", "UNIT_HEAL_PREDICTION", "UNIT_ABSORB_AMOUNT_CHANGED", "UNIT_HEAL_ABSORB_AMOUNT_CHANGED" }) do
    ...
    for _, ev in ipairs({ "GROUP_ROSTER_UPDATE", "PARTY_MEMBER_DISABLE", "PARTY_MEMBER_ENABLE", "READY_CHECK", "READY_CHECK_CONFIRM", "READY_CHECK_FINISHED", "PLAYER_TARGET_CHANGED", "INCOMING_RESURRECT_CHANGED", "INCOMING_SUMMON_CHANGED", "PORTRAITS_UPDATED" }) do
        frame:RegisterEvent(ev)
    end
    for _, ev in ipairs({ "UNIT_AURA", "UNIT_LEVEL", "UNIT_PHASE", "UNIT_HEALTH", "UNIT_MAXHEALTH", "UNIT_POWER_FREQUENT", "UNIT_MAXPOWER", "UNIT_NAME_UPDATE", "UNIT_MODEL_CHANGED", "UNIT_HEAL_PREDICTION", "UNIT_ABSORB_AMOUNT_CHANGED", "UNIT_HEAL_ABSORB_AMOUNT_CHANGED", "UNIT_THREAT_SITUATION_UPDATE", "UNIT_PORTRAIT_UPDATE" }) do
        frame:RegisterUnitEvent(ev, registerUnit)
    end
    frame:SetScript("OnEvent", frame.OnEvent)

    -- Erstelle Private Aura Frames
...
local function TogglePartyPreview(self)
    local total = MAX_PARTY_MEMBERS + 1  -- inklusive ggf. Spieler-Frame
    if self.previewMode then
        ...
        if not GW.settings.PARTY_PLAYER_FRAME then
            RegisterStateDriver(_G["GwPartyFrame" .. total], "visibility", "hide")
            RegisterStateDriver(_G["GwPartyPetFrame" .. total], "visibility", "hide")
        end
        ...
        for i = 1, total do
            ...
            if frame then
                ...
                frame:OnEvent("load")
                if frame.PetFrame and GW.settings.PARTY_SHOW_PETS then
