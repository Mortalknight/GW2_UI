...
function gw_manage_group_button()
    ...
    local fnGWMM_OnLoad = function(self)
        if
            ((IsInGroup() and gwGetSetting("RAID_STYLE_PARTY")) or IsInRaid()) and
    ...
    for i = 1, 40 do
        ...
        if DebuffLists[unit][i] then
            ...
            if DebuffLists[unit][i]["dispelType"] ~= nil and GW_DEBUFF_COLOR[DebuffLists[unit][i]["dispelType"]] ~= nil then
                _G["Gw" .. unit .. "DebuffItemFrame" .. i .. "DeBuffBackground"]:SetVertexColor(
                    GW_DEBUFF_COLOR[DebuffLists[unit][i]["dispelType"]].r,
                    GW_DEBUFF_COLOR[DebuffLists[unit][i]["dispelType"]].g,
                    GW_DEBUFF_COLOR[DebuffLists[unit][i]["dispelType"]].b
                )
            end
