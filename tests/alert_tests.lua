...
tests.MultiLoot = function(msg, editBox)
    ...
    gwDebug(link)
    C_Timer.After(1, function()
        LootAlertSystem:AddAlert("\124cffa335ee\124Hitem:18832::::::::::\124h[Brutality Blade]\124h\124r", 1, 1, 1, 1, false, false, 0, false, false)
...
function gwAlertTestsSetup()
    ...
    SlashCmdList['GW_TEST_ALERT'] = function(msg, editBox)
        local args = {}
        for arg in msg:gmatch('%w+') do tinsert(args, arg) end
        if not msg or not args or #args < 1 then
            gwDebug('use /gwtestalert <testname1> [testname2] ...')
            gwDebug('Multiple tests can be combined by specifying more than one test name (separated by spaces). This is a good way to test alert combinations. The alerts are added simultaneously.')
