...
-- Display settings
local USE_BLIZZARD_COLORS = false;
local USE_COMMA_FORMATING = false;
...
local function setElementData(self,critical,source)
    ...
    local activeColorTable = colorTable["gw"]
    if  USE_BLIZZARD_COLORS then
      activeColorTable = colorTable["blizzard"]
    end
...
local function formatDamageValue(amount)
  if USE_COMMA_FORMATING then
    return CommaValue(amount)
  end
  return amount
end
