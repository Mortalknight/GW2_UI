...
local function AlertContainerFrameOnEvent(self, event, ...)
    if event == "PLAYER_LEVEL_UP" and GW.GetSetting("ALERTFRAME_NOTIFICATION_LEVEL_UP") then
        ...
        if C_SpecializationInfo and C_SpecializationInfo.CanPlayerUsePVPTalentUI() and numNewPvpTalentSlots and numNewPvpTalentSlots > 0 then
        ...
        PlaySoundFile(GW.Libs.LSM:Fetch("sound", GW.GetSetting("ALERTFRAME_NOTIFICATION_LEVEL_UP_SOUND")), "Master")
    elseif event == "LEARNED_SPELL_IN_TAB" and GW.GetSetting("ALERTFRAME_NOTIFICATION_NEW_SPELL") and not self.ignoreNewSpells then
        local spellID = ...
        ...
        C_Timer.After(1.5, function()
            ...
            wipe(toastQueue)
            PlaySoundFile(GW.Libs.LSM:Fetch("sound", GW.GetSetting("ALERTFRAME_NOTIFICATION_NEW_SPELL_SOUND")), "Master")
        end)
        -- /run GW2_ADDON.AlertSystem:AddAlert(GetSpellInfo(48181), nil, LEVEL_UP_ABILITY, false, select(3, GetSpellInfo(48181)), false, 48181)
    elseif event == "PLAYER_SPECIALIZATION_CHANGED" and GW.GetSetting("ALERTFRAME_NOTIFICATION_NEW_SPELL") then
    ...
    elseif event == "UPDATE_PENDING_MAIL" and GW.GetSetting("ALERTFRAME_NOTIFICATION_NEW_MAIL") then
        ...
        if hasMail ~= newMail then
            ...
            if hasMail then
                ...
                PlaySoundFile(GW.Libs.LSM:Fetch("sound", GW.GetSetting("ALERTFRAME_NOTIFICATION_NEW_MAIL_SOUND")), "Master")
            end
        end
    elseif event == "UPDATE_INVENTORY_DURABILITY" and GW.GetSetting("ALERTFRAME_NOTIFICATION_REPAIR") then
        local current, max
        ...
        if showRepair and value < 20 then
            ...
            PlaySoundFile(GW.Libs.LSM:Fetch("sound", GW.GetSetting("ALERTFRAME_NOTIFICATION_REPAIR_SOUND")), "Master")
        end
    elseif event == "QUEST_ACCEPTED" and GW.GetSetting("ALERTFRAME_NOTIFICATION_PARAGON") then
        local questId = ...
        if PARAGON_QUEST_ID[questId] then
            ...
            PlaySoundFile(GW.Libs.LSM:Fetch("sound",GW.GetSetting("ALERTFRAME_NOTIFICATION_PARAGON_SOUND")), "Master")
        end
    elseif event == "VIGNETTE_MINIMAP_UPDATED" and GW.GetSetting("ALERTFRAME_NOTIFICATION_RARE") then
        ...
        if onMinimap then
            ...
            if vignetteGUID ~= self.lastMinimapRare.id then
                ...
                if time > (self.lastMinimapRare.time + 20) then
                    PlaySoundFile(GW.Libs.LSM:Fetch("sound", GW.GetSetting("ALERTFRAME_NOTIFICATION_RARE_SOUND")), "Master")
                    self.lastMinimapRare.time = time
                end
            end
        end
    elseif event == "CALENDAR_UPDATE_PENDING_INVITES" and GW.GetSetting("ALERTFRAME_NOTIFICATION_CALENDAR_INVITE") then
        if alertEvents() or alertGuildEvents() then
            PlaySoundFile(GW.Libs.LSM:Fetch("sound", GW.GetSetting("ALERTFRAME_NOTIFICATION_CALENDAR_INVITE_SOUND")), "Master")
        end
    elseif event == "CALENDAR_UPDATE_GUILD_EVENTS" and GW.GetSetting("ALERTFRAME_NOTIFICATION_CALENDAR_INVITE") then
    ...
    elseif event == "LFG_UPDATE_RANDOM_INFO" and GW.GetSetting("ALERTFRAME_NOTIFICATION_CALL_TO_ARMS") then
        ...
        if ((IsTank and forTank) or (IsHealer and forHealer) or (IsDamage and forDamage)) and not ingroup then
            if GetTime() - LFG_Timer > 20 then
                PlaySoundFile(GW.Libs.LSM:Fetch("sound", GW.GetSetting("ALERTFRAME_NOTIFICATION_CALL_TO_ARMS_SOUND")), "Master")
    ...
    elseif event == "COMBAT_LOG_EVENT_UNFILTERED" and (IsInRaid() or IsInGroup()) then
        ...
        if subEvent == "SPELL_CAST_SUCCESS" then
            if GW.GetSetting("ALERTFRAME_NOTIFICATION_MAGE_TABLE") and spellID == 190336 then -- Refreshment Table
                local name, _, icon = GetSpellInfo(190336)
                -- /run GW.AlertSystem:AddAlert(format("%s created a table of Conjured Refreshments.", "Hansi"), nil, GetSpellInfo(190336), false, select(3, GetSpellInfo(190336)), false)
                GW.AlertSystem:AddAlert(format(GW.L["%s created a table of Conjured Refreshments."], srcName), nil, name, false, icon, false)
                PlaySoundFile(GW.Libs.LSM:Fetch("sound", GW.GetSetting("ALERTFRAME_NOTIFICATION_MAGE_TABLE_SOUND")), "Master")
            end
        elseif subEvent == "SPELL_CREATE" then
            if GW.GetSetting("ALERTFRAME_NOTIFICATION_RITUAL_OF_SUMMONING") and spellID == 698 then -- Ritual of Summoning
                local name, _, icon = GetSpellInfo(698)
                -- /run GW.AlertSystem:AddAlert(format("%s is performing a Ritual of Summoning.", "Hansi"), nil, GetSpellInfo(698), false, select(3, GetSpellInfo(698)), false)
                GW.AlertSystem:AddAlert(format(GW.L["%s is performing a Ritual of Summoning."], srcName), nil, name, false, icon, false)
                PlaySoundFile(GW.Libs.LSM:Fetch("sound", GW.GetSetting("ALERTFRAME_NOTIFICATION_RITUAL_OF_SUMMONING_SOUND")), "Master")
            elseif GW.GetSetting("ALERTFRAME_NOTIFICATION_SPOULWELL") and spellID == 29893 then -- Soul Well
                local name, _, icon = GetSpellInfo(29893)
                -- /run GW.AlertSystem:AddAlert(format("%s created a Soulwell.", "Hansi"), nil, GetSpellInfo(29893), false, select(3, GetSpellInfo(29893)), false)
                GW.AlertSystem:AddAlert(format(GW.L["%s created a Soulwell."], srcName), nil, name, false, icon, false)
                PlaySoundFile(GW.Libs.LSM:Fetch("sound", GW.GetSetting("ALERTFRAME_NOTIFICATION_SPOULWELL_SOUND")), "Master")
            elseif GW.GetSetting("ALERTFRAME_NOTIFICATION_MAGE_PORTAL") and GW.MagePortals[spellID] then
                local name, _, icon = GetSpellInfo(spellID)
                -- /run GW.AlertSystem:AddAlert(format("%s placed a portal to %s.", "Hansi", GetSpellInfo(224871):gsub("^.+:%s+", "")), nil, GetSpellInfo(224871), false, select(3, GetSpellInfo(224871)), false)
                GW.AlertSystem:AddAlert(format(GW.L["%s placed a portal to %s."], srcName, name:gsub("^.+:%s+", "")), nil, name, false, icon, false)
                PlaySoundFile(GW.Libs.LSM:Fetch("sound", GW.GetSetting("ALERTFRAME_NOTIFICATION_MAGE_PORTAL_SOUND")), "Master")
            end
        end
    end
end
...
local function LoadAlertSystem()
    ...
    if GW.GetSetting("ALERTFRAME_SKIN_ENABLED") then
        -- Achievements
    ...
    if GW.GetSetting("ALERTFRAME_ENABLED") then
        ...
        local point = GW.GetSetting("AlertPos")
