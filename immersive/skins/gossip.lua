...
local function skinGossipOption(self)
    self.skinned = true
    self.Icon = _G[self:GetName() .. "GossipIcon"]
...
local function createCoordDebugInput(self, labelText, index)
    ...
    f.label:SetFont(UNIT_NAME_FONT, 16, "OUTLINE")
...
local function LoadGossipSkin()
    ...
    GossipGreetingScrollFrameScrollBar:GwSkinScrollBar()
    GossipGreetingScrollFrame:GwSkinScrollFrame()
    --nyi GW.HandleTrimScrollBar(GossipGreetingScrollFrameScrollBar)
    --nyi GW.HandleAchivementsScrollControls(GossipGreetingScrollFrameScrollBar)
    GossipFrameGreetingGoodbyeButton:Hide()
    GossipFrameGreetingGoodbyeButton:GwStripTextures()
    GossipFrameGreetingGoodbyeButton:GwSkinButton(false, true)
    ...
    greetings:SetFont(UNIT_NAME_FONT, 16, "OUTLINE")
    greetings:SetText("")
    ...
    hooksecurefunc("GossipFrameUpdate", function(frame)
        setGreetingsText(GossipGreetingText:GetText())

        GossipGreetingText:SetHeight(0)
        GossipGreetingText:SetPoint("TOPLEFT", GossipGreetingScrollChildFrame, "TOPLEFT", 10, 20)
        local numButtons = 0
        for _, button in next, { GossipGreetingScrollChildFrame:GetChildren() } do
            updateGossipOption(button)

            if button.type ~= "DIVIDER" then
