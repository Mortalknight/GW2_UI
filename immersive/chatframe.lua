...
local function setButtonPosition(frame)
    ...
    if frame.buttonSide == "right" then
        ...
        if not frame.isDocked then
            frame.Container:SetPoint("BOTTOMRIGHT", _G[name .. "EditBoxRight"], "BOTTOMRIGHT", 5, editbox:GetHeight() - 8)
        else
            frame.Container:SetPoint("BOTTOMRIGHT", _G[name .. "EditBoxRight"], "BOTTOMRIGHT", 5, 0)
        end
...
local function ChatThrottleBlockFlag(author, message, when)
    local msg = (author ~= PLAYER_NAME) and (tonumber(GW.settings.CHAT_SPAM_INTERVAL_TIMER) ~= 0) and PrepareMessage(author, message)

    local object = msg and throttle[msg]

    return object and object.time and object.count and object.count > 1 and (difftime(when, object.time) <= tonumber(GW.settings.CHAT_SPAM_INTERVAL_TIMER)), object
end
...
local function AddMessage(self, msg, infoR, infoG, infoB, infoID, accessID, typeID, alwaysAddTimestamp)
    local useGw2Style = GW.settings.CHAT_USE_GW2_STYLE

    if CHAT_TIMESTAMP_FORMAT and (GW.settings.CHAT_ADD_TIMESTAMP_TO_ALL or alwaysAddTimestamp) then
...
local function ChatFrame_MessageEventHandler(frame, event, arg1, arg2, arg3, arg4, arg5, arg6, arg7, arg8, arg9, arg10, arg11, arg12, arg13, arg14, arg15, arg16, arg17)
    ...
    if strsub(event, 1, 8) == "CHAT_MSG" then
        ...
        elseif chatType == "BN_INLINE_TOAST_BROADCAST_INFORM" then
            ...
            frame:AddMessage(body, info.r, info.g, info.b, info.id, accessID, typeID, not GW.settings.CHAT_ADD_TIMESTAMP_TO_ALL)
        end
...
local function styleChatWindow(frame)
    ...
    if not frame.hasContainer and (isDocked == 1 or (isDocked == nil and frame:IsShown())) then
        ...
        fmGCC:ClearAllPoints()
        fmGCC:SetPoint("TOPLEFT", frame, "TOPLEFT", -35, 5)
        if not frame.isDocked then
            fmGCC:SetPoint("BOTTOMRIGHT", _G[name .. "EditBoxRight"], "BOTTOMRIGHT", 5, editbox:GetHeight() - 8)
        else
            fmGCC:SetPoint("BOTTOMRIGHT", _G[name .. "EditBoxRight"], "BOTTOMRIGHT", 5, 0)
        end
...
local function LoadChat()
    ...
    if FriendsMicroButton then
        ...
        --FriendsMicroButton.QueueCount:GwKill()
        local _, _, fontFlags = FriendsMicroButtonCount:GetFont()
        FriendsMicroButtonCount:SetFont(_, 12, fontFlags)
        ...
        if GW.settings.CHAT_SOCIAL_LINK then
            --FriendsMicroButton.Toast:GwKill()
            --FriendsMicroButton.Toast2:GwKill()
        end
    ...
    hooksecurefunc("FCF_DockUpdate", function()
        for _, frameName in ipairs(CHAT_FRAMES) do
            ...
            if not frame.hasContainer and (isDocked == 1 or (isDocked == nil and frame:IsShown())) then
                ...
                fmGCC:SetPoint("TOPLEFT", frame, "TOPLEFT", -35, 5)
                if not frame.isDocked then
                    fmGCC:SetPoint("BOTTOMRIGHT", _G[frameName .. "EditBoxRight"], "BOTTOMRIGHT", 5, editbox:GetHeight() - 8)
                else
                    fmGCC:SetPoint("BOTTOMRIGHT", _G[frameName .. "EditBoxRight"], "BOTTOMRIGHT", 5, 0)
                end
                if not frame.isDocked then fmGCC.EditBox:Hide() end
                frame.Container = fmGCC
                frame.hasContainer = true
    ...
    do
		local menuBackdrop = function(s)
			s:GwCreateBackdrop(GW.skins.constBackdropFrame)
		end
