...
local function keyBindPrompt(text)
    bind.promtFrame.string:SetText(text)
...
local function LoadHoverBinds()
    ...
    bind.promtFrame.string:SetFont(UNIT_NAME_FONT, 14, "")
    bind.promtFrame.string:SetTextColor(1, 1, 1)
    bind.promtFrame.acceptButton:SetText(SAVE)
    bind.promtFrame.cancelButton:SetText(CANCEL)
    bind.promtFrame.acceptButton:SetScript("OnClick", fmGWKB_accept_OnClick)
    bind.promtFrame.cancelButton:SetScript("OnClick", fmGWKB_cancel_OnClick)
end
