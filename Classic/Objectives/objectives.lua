...
local function CreateObjectiveNormal(name, parent)
    local f = CreateFrame("Frame", name, parent, "GwQuesttrackerObjectiveTemplate")
...
local function CreateTrackerObject(name, parent)
    local f = CreateFrame("Button", name, parent, "GwObjectivesBlockTemplate")
...
local function updateQuest(block, quest)
    ...
    if quest.questId then
        ...
        --Quest item
        GW.CombatQueue_Queue("update_quest_item_button", UpdateQuestItem, {block})

        if Questie and Questie.started then
            local xpReward = QuestieLoader:ImportModule("QuestXP"):GetQuestLogRewardXP(quest.questId, false)

            if xpReward and GW.settings.QUESTTRACKER_SHOW_XP and GW.mylevel < GetMaxPlayerLevel() then
...
local function updateQuestLogLayout(self)
    ...
    for _, quest in pairs(sorted) do
        if ((GetCVar("autoQuestWatch") == "1" and not GW2UI_QUEST_WATCH_DB.AutoUntrackedQuests[quest.questId]) or (GetCVar("autoQuestWatch") == "0" and GW2UI_QUEST_WATCH_DB.TrackedQuests[quest.questId])) then
            ...
            counter = counter + 1
            GW.CombatQueue_Queue("update_quest_item_button_position", updateQuestItemPositions, {savedHeight, block})
    ...
    for i = (GwQuesttrackerContainerQuests.collapsed and 0 or #GW.trackedQuests + 1), 25 do
        if _G["GwQuestBlock" .. i] then
            ...
            GW.CombatQueue_Queue("update_quest_item_button", UpdateQuestItem, {_G["GwQuestBlock" .. i]})
        end
    end
...
local function LoadObjectivesTracker()
    ...
    fTracker.LayoutChanged = QuestTrackerLayoutChanged
    ...
    Mixin(fNotify, GwObjectivesTrackerNotificationMixin)
    fNotify:InitModule()
