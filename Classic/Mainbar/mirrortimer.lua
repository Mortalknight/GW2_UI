...
local function MirrorTimer_Show(timer, value, maxvalue, scale, paused, label)
    ...
    dialog.bar:SetStatusBarTexture("Interface/Addons/GW2_UI/textures/hud/castinbar-white.png")
    dialog.bar:SetStatusBarColor(color.r, color.g, color.b)
    dialog:Show()
end
GW.MirrorTimer_Show = MirrorTimer_Show

local function mirrorTimerFrame_OnEvent(self, event, ...)
...
local function LoadMirrorTimers()
    for i = 1, _G.MIRRORTIMER_NUMTIMERS do
        ...
        mirrorTimer.bar.name:GwSetFontTemplate(UNIT_NAME_FONT, GW.Enum.TextSizeType.Normal, "OUTLINE")
