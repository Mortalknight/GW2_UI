...
local function grabDefaultSlots(slot, anchor, parent, size)
    ...
    slot.IconOverlay:SetTexCoord(0.1, 0.9, 0.1, 0.9)
    slot.IconOverlay:ClearAllPoints()
    slot.IconOverlay:SetPoint("TOPLEFT", slot, "TOPLEFT", -1, 1)
    slot.IconOverlay:SetPoint("BOTTOMRIGHT", slot, "BOTTOMRIGHT", 1, -1)
    
    slot.IconBorder:SetTexture("Interface/AddOns/GW2_UI/textures/bag/bagitemborder")
    slot.IconBorder:ClearAllPoints()
    slot.IconBorder:SetPoint("TOPLEFT", slot, "TOPLEFT", -1, 1)
    slot.IconBorder:SetPoint("BOTTOMRIGHT", slot, "BOTTOMRIGHT", 1, -1)
    slot.IconBorder:SetParent(slot)

    local norm = slot:GetNormalTexture()
    norm:SetTexture(nil)
