...
function gwSpellBookSpell_onDrag(self)
    ...
    if
        (not slot or slot > MAX_SPELLS or not _G[self:GetName() .. "IconTexture"]:IsShown() or
            (slotType == "FUTURESPELL"))
     then
        return
    end
...
function gw_register_spellbook_window()
    ...
    for i = 1, 21 do
        ...
        f.title:SetFont(UNIT_NAME_FONT, 12)
        f.sub:SetFont(UNIT_NAME_FONT, 11)
        f.sub:SetTextColor(0.8, .8, .8, 1)

        f.title:SetShadowColor(0, 0, 0, 1)
        f.sub:SetShadowColor(0, 0, 0, 1)
