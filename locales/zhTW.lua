-- zhTW localization
local L = LibStub("AceLocale-3.0"):NewLocale("GW2_UI", "zhTW")
if not L then return end

--Fonts
L["FONT_NORMAL"] = "Interface/AddOns/GW2_UI/fonts/chinese-font.ttf"
L["FONT_BOLD"] = "Interface/AddOns/GW2_UI/fonts/chinese-font.ttf"
L["FONT_NARROW"] = "Interface/AddOns/GW2_UI/fonts/chinese-font.ttf"
L["FONT_NARROW_BOLD"] = "Interface/AddOns/GW2_UI/fonts/chinese-font.ttf"
L["FONT_LIGHT"] = "Interface/AddOns/GW2_UI/fonts/chinese-font.ttf"
L["FONT_DAMAGE"] = "Interface/AddOns/GW2_UI/fonts/chinese-font.ttf"

--classic ones
L["FPS "] = "每秒幀數 "
L["Latency (Home) "] = "網路延遲 (本地) "
L["Latency (World) "] = "網路延遲 (世界) "
L["Bandwidth (Download) "] = "頻寬 (下載) "
L["Bandwidth (Upload) "] = "頻寬 (上傳) "
L["Memory for Addons: "] = "插件記憶體用量: "
L["Future Spells"] = "Future Spells"
L["Ranged"] = "遠程"
L["5 secound rule: display remaning time"] = true
L["Energy/Mana Ticker"] = true
L["Show Energy/Mana Ticker only in combat"] = true
L["All key bindings have been saved."] = true
L["All newly set key bindings have been discarded."] = true
L["Hover your mouse over any action button to bind it. Press the esc key or right click to clear the current action button's keybinding."] = true
L["Trigger"] = true
L["Key"] = true
L["All key bindings cleared for"] = true
L["bound to"] = true
L["Sold Junk for: %s"] = true
L["Show Quest XP on Quest Tracker"] = true

--Strings
L[" (Resting)"] = " (休息中)"
L["%s and then %s"] = "先%s再%s"
L["%s can be collected"] = "%s 可以收集"
L["%s created a Soulwell."] = "%s 發糖囉～"
L["%s created a table of Conjured Refreshments."] = "%s 開了餐桌，快來領便當哦～"
L["%s Indicator"] = "%s圖示"
L["%s is performing a Ritual of Summoning."] = "%s 正在開門拉人，請幫忙點門。"
L["%s placed a portal to %s."] = "%s 開了%s傳送門。"
L["%s slot needs to repair, current durability is %d."] = "%s需要修理，目前耐久值為  %d。"
L["%s will start in %s!"] = "%s將在 %s 後開始!"
L[": Show numbers with commas"] = ": 數字顯示逗號"
L[": Use Blizzard colors"] = ": 使用暴雪顏色"
L["|cffFFFFFFControl + Left Click:|r Change Loadout"] = "|cffFFFFFFCtrl+左鍵:|r 更改配置"
L["|cffFFFFFFLeft Click:|r Show Talent Specialization UI"] = "|cffFFFFFF左鍵:|r 打開天賦視窗"
L["|cffFFFFFFRight Click:|r Change Loot Specialization"] = "|cffFFFFFF右鍵:|r 切換優先拾取專精"
L["|cffFFFFFFShift + Left Click:|r Change Talent Specialization"] = "|cffFFFFFFShift+左鍵:|r 切換天賦專精"
L["<Assign specializations>"] = "<分配專精>"
L[ [=[A |cFFFF0000major|r update is available.
It's strongly recommended that you update.]=] ] = [=[有新版本! 包含|cFFFF0000重大更新|r可供下載。
強烈建議更新插件。]=]
L["A list of auras that should never be shown."] = "永遠不要顯示的光環名稱清單，使用逗號分隔。"
L["A list of buffs that should only be shown when they are missing."] = "只有缺少時才要顯示的光環名稱清單，使用逗號分隔。"
L["Action Button Labels"] = "顯示快捷鍵"
L["Add and remove profiles."] = "新增和移除設定檔。"
L["Add LFG group info to tooltip."] = "將隨機隊伍資訊加入到浮動提示資訊中。"
L["Add timestamp to all messages"] = "所有訊息加上時間標記"
L["Add trackers for world events in the bottom of world map."] = "在世界地圖最下方加入世界事件的追蹤器。"
L["AddOn Memory:"] = "插件記憶體:"
L["Adds descriptions for mythic keystone properties to their tooltips."] = "在傳奇鑰石的浮動提示資訊中加入詞綴內容說明。"
L["Adjust Skin settings."] = "調整外觀設定。"
L["Advanced Casting Bar"] = "進階施法條"
L["Advanced Tooltips"] = "進階浮動提示資訊"
L["AFK Mode"] = "AFK 模式"
L["Alert Frames"] = "提醒框架"
L["Alert Second"] = "提醒秒數"
L["Alert Timeout"] = "提醒逾時"
L["Alert will be disabled after the set value (hours)."] = "設定數值 (小時) 後將會停用提醒。"
L["Alert will be triggered when the remaining time is less than the set value."] = "剩餘時間小於設定的數值時，將會觸發提醒。"
L["All nets can be collected"] = "所有漁網都可以收集。"
L["Allied Races"] = "同盟種族"
L["Allow the chat to fade when not in use."] = "非使用中時淡出聊天視窗。"
L["Alt + Left Click"] = "Alt+左鍵"
L["Always show Minimap details."] = "永遠顯示小地圖詳細資訊內容。"
L["Always Show Realm"] = "總是顯示伺服器"
L["Anchor the tooltips to the cursor."] = "在滑鼠游標的位置顯示浮動提示資訊。"
L["Animated"] = "動畫"
L["Applies the UI scale to all frames, which can be scaled in 'Move HUD' mode."] = "介面縮放大小套用到所有在 '移動介面' 模式中能夠縮放大小的框架。"
L["Apply UI to all"] = "套用到所有使用者介面"
L["Are you sure you want to delete the selected layout?"] = "是否確定要刪除選擇的版面配置?"
L["Are you sure you want to delete this profile?"] = "是否確定要刪除這個設定檔?"
L[ [=[Are you sure you want to load the default settings?

All previous settings will be lost.]=] ] = [=[是否確定要恢復為預設設定?

將會失去先前更改過的所有設定。]=]
L["Ascending"] = "遞增"
L["Assign layouts to a spec. The layout will be automatically changed on a spec switch.\\n\\nLayouts has always priority for profile settings."] = "將版面配置分配給專精。切換專精時會自動更改版面配置。\\n\\n版面配置會永遠優先用於設定檔。"
L["Attempt to create URL links inside the chat."] = "嘗試在聊天中建立 URL 連結。"
L["Aura Style"] = "光環樣式"
L["Auras per row"] = "每列光環數量"
L["Auto Repair"] = "自動修裝"
L["Automatically repair using the following method when visiting a merchant."] = "和商人對話時，依據下列規則自動修理裝備。"
L["Bandwidth"] = "頻寬"
L["Bar"] = "條上"
L["Blizzard Class Colors"] = "暴雪預設職業顏色"
L["Bonus Net"] = "額外漁網"
L["Boss Button"] = "首領按鈕"
L["Boss frames: Separate Energy Bar"] = "首領框架: 分離能量條。"
L["Bottom"] = "下方"
L["Buff size"] = "增益圖示大小"
L["Buffs"] = "增益"
L["Button Spacing"] = "按鈕間距"
L["By growth direction"] = "依據延伸方向"
L["By position on screen"] = "依據畫面上的位置"
L["Calendar invite"] = "行事曆邀請"
L["Can be collected"] = "可以收集"
L["Can be set"] = "可以設定"
L["Center"] = "中間"
L["Center of screen"] = "畫面中心"
L["Change the HUD size."] = "更改介面大小。"
L["Change the Minimap size."] = "更改小地圖大小。"
L["Changelog"] = "更新資訊"
L["Changes the chat font, timestamp color and name display"] = "更改聊天字體、時間戳記顏色和名字顯示方式。"
L["Chromie Time Frame"] = "克羅米時間框架"
L["Class Color Mentions"] = "提到時顯示職業顏色"
L["Class Power"] = "職業特殊能量"
L["Class Totems"] = "職業圖騰"
L["Class Totems Growth Direction"] = "職業圖騰延伸方向"
L["Class Totems Sorting"] = "職業圖騰排序"
L["Classic combat text anchoring"] = "經典戰鬥文字對齊"
L["Click to open Emoticon Frame"] = "點一下打開表情圖案視窗"
L["Click to toggle raid frame preview and cycle through different group sizes."] = "點一下切換預覽團隊框架，多次點擊切換不同的隊伍大小。"
L["Collapse all objectives in M+"] = "M+ 時收合所有任務目標"
L["Combat Override Key"] = "戰鬥中需要按住"
L["Combat Repeat"] = "戰鬥重複字數"
L["Community Feast"] = "集體盛宴"
L["Compact Icons"] = "小圖示"
L["Complete"] = "結束"
L["Conduits"] = "靈印"
L["Cooking"] = "烹飪"
L["Coordinates"] = "坐標"
L["Copy of"] = "複製的"
L["Created by: "] = [=[
建立角色: ]=]
L["Created: "] = "建立時間: "
L["Credits"] = "製作群"
L["Crit modifier"] = "致命一擊修正"
L["Ctrl & Shift Click: Toggle CPU Profiling"] = "Ctrl+Shift+左鍵: 切換 CPU 分析"
L["Ctrl + Click to toggle override"] = "Ctrl + 左鍵點擊來取代"
L["Current Mount"] = "目前坐騎"
L["Cursor Anchor"] = "游標指向點"
L["Cursor Anchor Left"] = "游標左側"
L["Cursor Anchor Offset X"] = "跟隨游標水平偏移"
L["Cursor Anchor Offset Y"] = "跟隨游標垂直偏移"
L["Cursor Anchor Right"] = "游標右側"
L["Cursor Anchor Type"] = "跟隨游標位置"
L["Cursor Tooltips"] = "浮動提示資訊跟隨游標"
L["Custom buff"] = "自訂增益"
L["Daily Reset"] = "每日重置"
L["Debuff size"] = "減益圖示大小"
L["Debuffs"] = "減益"
L["Decode"] = "解析"
L["Default Settings"] = "載入預設值"
L["Deficit:"] = "虧損:"
L["Defines how the group is sorted"] = "隊伍如何排序"
L["Defines the sort order of the selected sort method"] = "所選排序方式的排列順序"
L["Desaturate icon"] = "圖示去飽和度"
L["Desaturate icon if the event is completed in this week."] = "本週已完成的事件圖示去飽和度。"
L["Descending"] = "遞減"
L["Developed by"] = "程式設計"
L["Different Than Own"] = "和我不同的"
L["Dimmed"] = "變暗"
L["Disable FPS tooltip"] = "停用 FPS 浮動提示資訊"
L["Dispell > Important"] = "可驅散 > 重要"
L["Display a country flag based on the unit's language"] = "根據玩家所使用的語言顯示國旗。"
L["Display a separate raid grid for groups from 1 to 10 players"] = "用分開的團隊框架顯示1-10人隊伍"
L["Display a separate raid grid for groups from 11 to 25 players"] = "用分開的團隊框架顯示11-25人隊伍"
L["Display all of the target's debuffs."] = "顯示目標身上全部的減益效果。"
L["Display Average Item Level"] = "顯示平均物品等級"
L["Display current mount the unit is riding."] = "顯示單位目前正在騎的坐騎資訊。"
L["Display emotion icons in chat"] = "在聊天顯示表情圖案"
L["Display gems and enchants on the GW2 character panel"] = "GW2 的角色視窗顯示珠寶和附魔"
L["Display guild ranks if a unit is a member of a guild."] = "如果單位有公會，顯示會階。"
L["Display health as a percentage. Can be used as well as, or instead of Health Value."] = "顯示血量百分比，可以單獨顯示或和血量數值一起顯示。"
L["Display how many of a certain item you have in your possession."] = "顯示某樣物品你擁有多少數量。"
L["Display LFG Icons in group chat."] = "在隊伍聊天顯示尋求組隊圖示。"
L["Display player titles."] = "顯示玩家頭銜。"
L["Display Portrait Damage"] = "頭像上顯示傷害"
L["Display Portrait Damage on this frame"] = "在此框架頭像上面顯示傷害數字。"
L["Display the average item level instead of prestige level for friendly units."] = "友方單位顯示平均物品等級，而不是聲望等級。"
L["Display the class color as the health bar."] = "血量條使用職業顏色。"
L["Display the focus target frame."] = "顯示專注目標的目標框架。"
L["Display the hyperlink tooltip while hovering over a hyperlink."] = "滑鼠指向連結時，顯示連結的浮動提示資訊。"
L["Display the power bars on the raid units."] = "顯示隊友的能量條。"
L["Display the target's buffs."] = "顯示目標的增益效果。"
L["Display the target's debuffs that you have inflicted."] = "顯示目標身上由你所施放的減益效果。"
L["Display the unit role in the tooltip."] = "在浮動提示資訊中顯示單位的角色職責。"
L["Displays additional information in the tooltip (further information is displayed when the SHIFT key is pressed)"] = "在浮動提示中顯示更多資訊 (按住 SHIFT 鍵來顯示)"
L["Displays the player character's gender."] = "顯示玩家角色的性別。"
L["Displays the Target Markers on the Raid Unit Frames"] = "在團隊框架上面顯示目標標記圖示。"
L["Dodge Bar Ability"] = "位移條技能"
L["Down"] = "下"
L["Down and right"] = "右下"
L["Download"] = "下載"
L["Dragonbane Keep"] = "龍禍要塞"
L["Dungeon & Raid Debuffs"] = "地城 & 團隊減益"
L["Dynamic HUD"] = "動態 HUD 背景"
L["Earned:"] = "賺取:"
L["Edit %s raid aura indicator."] = "編輯%s團隊光環指示圖示。"
L["Edit chat settings."] = "編輯聊天設定。"
L["Edit objectives settings."] = "編輯任務目標設定。"
L["Edit raid aura indicators."] = "編輯團隊框架上面增益和減益效果的指示圖示。"
L["Edit raid buff bar."] = "編輯團隊增益列。"
L["Edit the group settings."] = "編輯隊伍設定。"
L["Edit the HUD modules."] = "編輯 HUD 介面模組。"
L["Edit the modules in the Heads-Up Display for more customization."] = "編輯 HUD 模組，打造喜愛的介面。"
L["Edit the party and raid options to suit your needs."] = "編輯小隊和團隊選項以符合你的需求。"
L["Edit the target frame settings."] = "編輯選取目標框架的設定。"
L["Edit which buffs and debuffs are shown."] = "編輯要顯示哪些增益和減益效果。"
L["Emotion Icons"] = "表情圖案"
L["Empty slots alpha"] = "空欄位透明度"
L["Enable and disable components"] = "啟用或停用元件"
L["Enable Maintank grid"] = "啟用主坦克框架"
L["Enable or disable the action button assignment text"] = "啟用或停用快捷列上的按鍵提示文字。"
L["Enable or disable the advanced casting bar."] = "啟用或停用進階的施法條。"
L["Enable or disable the dynamically changing HUD background."] = "啟用或停用動態變換主要快捷列後方的 HUD 背景。"
L["Enable or disable the modules you need and don't need."] = "啟用或停用需要和不需要使用的模組。"
L["Enable or disable the quest tracker compass."] = "啟用或停用任務追蹤清單的羅盤。"
L["Enable Raid 10 grid"] = "啟用10人團隊框架"
L["Enable Raid 25 grid"] = "啟用25人團隊框架"
L["Enable Raid pet grid"] = "啟用寵物團隊框架"
L["Enable the alternate class powers."] = "顯示職業特殊能量。"
L["Enable the focus target frame replacement."] = "取代專注目標框架。"
L["Enable the GW2 style casting bar."] = "使用 GW2 UI 風格的施法條。"
L["Enable the health bar replacement."] = "取代血條。"
L["Enable the immersive questing view."] = "使用身歷其境的任務畫面。"
L["Enable the improved chat window."] = "使用加強型的聊天視窗。"
L["Enable the revamped and improved quest tracker."] = "使用重新製作加強型的任務追蹤清單。"
L["Enable the talents, specialization, and spellbook replacement."] = "取代預設的天賦、專精和法術書視窗。"
L["Enable the target frame replacement."] = "取代選取目標框架。"
L["Enable the target of target frame."] = "顯示選取目標的目標框架。"
L["Enable the unified inventory interface."] = "使用整合背包介面。"
L["Enabling this allows raid-wide sorting however you will not be able to distinguish between groups."] = "啟用此選項允許對整個團隊進行排序，但是將無法區分隊伍。"
L["Enter the spell ID of the buff you wish to track. Only one spell ID is supported. To find the spell ID of the buff you want to track, enable IDs in the tooltip settings and mouse over the icon in your aura bar."] = "輸入你想要監控的增益效果法術 ID，只能輸入一個法術 ID。要查看法術 ID，請在浮動提示資訊設定中啟用顯示 ID，然後將滑鼠指向光環列的圖示。"
L[ [=[Enter the spell ID which should be tracked by the dodge bar.
If no ID is entered, the default abilities based on your specialization and talents are tracked.]=] ] = [=[輸入位移技能條要追蹤的法術 ID。
如果沒有輸入 ID，會依據你的專精和天賦來追蹤預設的技能。]=]
L["Error decoding profile: Invalid or corrupt string!"] = "解析設定檔錯誤: 字串無效或已損毀!"
L["Error importing profile: Invalid or corrupt string!"] = "匯入設定檔錯誤: 字串無效或已損毀!"
L["Export"] = "匯出"
L["Export Profile"] = "匯出設定檔"
L["Extended Vendor"] = "加大商人視窗"
L["Extra Frame Options"] = "更多框架選項"
L["Fade Action Bars"] = "淡出快捷列"
L["Fade Chat"] = "淡出聊天視窗"
L["Fade Group Manage Button"] = "淡出隊伍管理按鈕"
L["Fade Menu Bar"] = "淡出微型選單"
L["Feast"] = "盛宴"
L["Filter"] = "過濾方式"
L["Fishing Net"] = "漁網"
L["Fishing Nets"] = "漁網"
L["Flash taskbar on reminder"] = "提醒時閃爍工作列"
L["Fonts"] = "字型"
L["G"] = "公"
L["Gender"] = "性別"
L["Gossip Frame"] = "對話"
L["Grays"] = "垃圾"
L["Greyed out"] = "變灰"
L["Grid Size:"] = "格子大小:"
L["Group By"] = "隊伍依據"
L["Group Frames"] = "隊伍框架"
L["Group Spacing"] = "隊伍間距"
L["Grouping & Sorting"] = "隊伍 & 排序"
L["Groups Per Row/Column"] = "每行/列顯示幾個隊伍"
L["GW2 chat message style"] = "GW2 聊天訊息樣式"
L["GW2 floating combat text style"] = "GW2 浮動戰鬥文字樣式"
L["GW2 UI installation"] = "安裝 GW2 UI "
L[ [=[GW2 UI is a full user interface replacement. We have built the user interface with a modular approach, this means that if you dislike a certain part of the addon - or have another you prefer for that function - you can just disable that part, while keeping the rest of the interface intact.
Some of the modules available to you are an immersive questing window, a full inventory replacement, as well as a full character window replacement. There are many more that you can enjoy, just take a look in the settings menu to see what's available to you!]=] ] = [=[GW2 UI 是一套完整的使用者介面，用來取代原本的遊戲介面。我們使用模組化的方式來建立這個使用者介面，意思是說，如果你不喜歡插件的某個部分，或是偏好使用其他插件的同類型功能，只要單獨停用該部分即可，仍然可以保持介面銜接的完整性。
GW2 UI 提供的模組包含身歷其境的任務視窗，完整的背包和角色視窗替換介面，只要看一下設定選項，將會發現更多可供使用、你會喜愛的功能!]=]
L["GW2 UI Update"] = "GW2 UI 更新"
L["has appeared on the Minimap!"] = "已經出現在小地圖上!"
L["Health Bar Position"] = "血量條位置"
L["Health Globe"] = "血球"
L["Hide Blizzard dragon riding vigor"] = "隱藏遊戲內建的飛龍騎術活力"
L["Hide Editbox"] = "隱藏文字輸入框"
L["Hide Empty Slots"] = "隱藏空欄位"
L["Hide grid"] = "隱藏格子"
L["Hide the chat editbox when not in focus."] = "平時隱藏文字輸入框。"
L["Hide the empty action bar slots."] = "隱藏空的快捷列欄位。"
L["Hold Shift: Memory Usage"] = "按住 Shift 鍵: 記憶體使用量"
L["Home Latency:"] = "本地延遲:"
L["Home Protocol:"] = "本地協定:"
L["Horizontal"] = "水平"
L["Horizontal Spacing"] = "水平間距"
L["HUD Scale"] = "介面縮放"
L["Hyperlink Hover"] = "滑鼠指向連結"
L["I"] = "副"
L["If both scales could apply to a debuff, which one should be used"] = "兩種縮放大小都符合套用條件時，要使用哪一種。"
L["If enabled, an animated border will surround the missing raid buffs"] = "啟用時，缺少的團隊增益效果會有動畫效果的邊框。"
L["If enabled, enabled bosses’ energy bars will be shown separately from their health bar."] = "啟用時，作用中的首領能量條會和血量條分開顯示。"
L["If enabled, the above settings will apply to buffs you have, instead of buffs you are missing"] = "啟用時，上面的設定會套用到你已有的增益效果，而不是缺少的。"
L["If enabled, this will show both the stylised party frames as well as the grid frame. This setting has no effect if '%s' is enabled."] = "啟用時，會同時顯示美化的隊伍框架和團隊框架。如果已啟用 '%s' 則此選項不會有效果。"
L["Ignored Auras"] = "忽略光環"
L["IL"] = "長"
L["Immersive Questing"] = "身歷其境的任務對話"
L["Import"] = "匯入"
L["Import Profile"] = "匯入設定檔"
L["Import string successfully decoded!"] = "匯入字串解析完成!"
L["Import string successfully imported!"] = "字串匯入完成!"
L["Important & dispellable debuff scale priority"] = "重要 & 可驅散的減益效果縮放優先順序"
L["Important > Dispell"] = "重要 > 可驅散"
L["In group or in raid"] = "在隊伍或團隊中"
L["In raid"] = "在團隊中"
L["Incompatible Addons behavior Overridden. Needs a reload to take effect."] = "已被不相容的插件功能取代，重新載入介面後才會生效。"
L["Index"] = "索引"
L["Indicate whether buffs you cast yourself should be separated before or after"] = "你所施放的增益要在前面或後面分開顯示"
L["Installation"] = "安裝程序"
L["Installation Complete"] = "安裝完成"
L["Interval"] = "間隔"
L["Invert focus frame"] = "反轉專注目標框架"
L["Invert raid buff bar"] = "反轉團隊增益列"
L["Invert target frame"] = "反轉選取目標框架"
L["is looking for members"] = "正在徵求會員"
L["Iskaaran Fishing Net"] = "伊斯凱拉漁網"
L["Item Count"] = "物品數量"
L["Join Discord"] = "加入 Discord"
L["joined a group"] = "已加入群組"
L["Keyword Alert"] = "關鍵字提醒"
L["Keyword highlight color"] = "關鍵字顯著標示顏色"
L["Keywords"] = "關鍵字"
L["Last updated: "] = [=[
上次更新: ]=]
L["Left"] = "左側"
L["Left click on a moverframe to show extra frame options"] = "點一下移動框架顯示更多選項。"
L["Left Click to toggle higher precision coordinates."] = "點一下切換坐標的精確度。"
L["Legacy: Sorted by duration but auras can't cancel via right click in combat\\nSecure: Not sorted but auras can cancel via right click in combat"] = "傳統: 依照時間排序，但是戰鬥中無法點右鍵取消光環。\\n安全: 不會排序，但是戰鬥中可以點右鍵取消光環。"
L["Limit the number of rows"] = "限制橫列數"
L["List of words to color in chat if found in a message. If you wish to add multiple words you must seperate the word with a comma. To search for your current name you can use %MYNAME%.\\n\\nExample:\\n%MYNAME%, Heal, Tank"] = "列出聊天訊息中要變色的文字，輸入多個字時請用逗號分隔。要找出你當前角色的名字可以使用 %MYNAME%。\\n\\n範例:\\n%MYNAME%, 治療, 坦克"
L["Load"] = "載入"
L["Load the default addon settings to the current profile."] = "載入插件的預設設定到當前設定檔。"
L["Loadouts"] = "配置"
L["Localised by"] = "翻譯"
L["Lock HUD"] = "鎖定介面"
L["Looking for Group notifications"] = "隊伍搜尋器通知"
L["Loot to leftmost Bag"] = "拾取到最左側的背包"
L["Mage portal"] = "法師傳送門"
L["Mage table"] = "法師餐桌"
L["Main Action Bar"] = "主要快捷列"
L["Main Bar Range Indicator"] = "主要快捷列超出範圍指示"
L["Map Coordinates"] = "地圖坐標"
L["Mark Quest Reward"] = "標示任務獎勵"
L["Marks the most valuable quest reward with a gold coin."] = "最有價值的任務獎勵加上金錢圖示。"
L["Max Wraps"] = "最大換行數"
L["Maximum lines of 'Copy Chat Frame'"] = "'複製聊天視窗' 的最大行數"
L["Micro Bar"] = "微型選單"
L["Minimap details"] = "小地圖資訊"
L["Minimap Scale"] = "小地圖縮放"
L["Misc Frames"] = "其他"
L["Missing Buffs"] = "缺少光環"
L["Missing Raid Buffs"] = "缺少的團隊增益"
L["Missing Raid Buffs Bar"] = "缺少的團隊增益列"
L["Modifier for IDs"] = "顯示 ID 的輔助鍵"
L["Modifier to hold to show the tooltip in combat."] = "在戰鬥中要顯示浮動提示資訊的輔助按鍵。"
L["Modify the focus frame settings."] = "調整專注目標框架的設定。"
L["Modify the player frame settings."] = "調整玩家框架的設定。"
L["Modify the target frame settings."] = "調整選取目標框架的設定。"
L["Modules"] = "模組"
L["Move and resize the player auras."] = "移動和縮放玩家光環。"
L["Move HUD"] = "移動介面"
L["MoveAnything bag handling disabled."] = "已停用版面配置插件 MoveAnything 的背包調整功能。"
L["Net #%d"] = "漁網 #%d"
L["Net %s can be collected"] = "可以收集 %s"
L["New Bag Name"] = "新背包名稱"
L["New layout name:"] = "新的版面配置名稱:"
L["New mail"] = "新郵件"
L["New spell"] = "新法術"
L["New update available containing new features."] = "有新版本! 包含新功能的更新可供下載。"
L["New update available for download."] = "有新版本! 已有更新可供下載。"
L["Next Event"] = "下個事件"
L["No extra frame options for '%s'"] = "'%s' 沒有更多的框架選項"
L["No Guild"] = "沒有公會"
L["No Nets Set"] = "沒有漁網套裝"
L["No Sorting"] = "不要排序"
L["Not Set"] = "沒有套裝"
L["Number of columns in the two extra right-hand action bars."] = "兩個右方額外快捷列的直行數。"
L["Number of messages you scroll for each step."] = "每次捲動的訊息行數。"
L["Number of repeat characters while in combat before the chat editbox is automatically closed, set to zero to disable."] = "戰鬥中重複輸入多少個字後會自動關閉聊天輸入框，設為 0 會停用此功能。"
L["Number of time in seconds to scroll down to the bottom of the chat window if you are not scrolled down completely."] = "沒有完全向下捲動時，捲動到聊天視窗最下面需要的時間 (以秒為單位)。"
L["O"] = "幹"
L["One or more of the changes you have made require a UI reload."] = "你所做的變更需要重新載入介面後才會生效。"
L["Only displays the debuffs that you are able to dispell."] = "只顯示你可以驅散的減益效果。"
L["Only on Mouse Over"] = "只有滑鼠指向時"
L["Only takes effect if the option 'Cursor Tooltips' is activated"] = "只有啟用 '浮動提示資訊跟隨游標' 時才有效果。"
L["Only takes effect if the option 'Cursor Tooltips' is activated and the cursor anchor is NOT 'Cursor Anchor'"] = "只有啟用 '浮動提示資訊跟隨游標'，而且跟隨游標位置不是 '游標指向點' 時才有效果。"
L["Other's First"] = "其他人的優先"
L["Out of combat"] = "非戰鬥中"
L["P"] = "隊"
L["Paragon"] = "巔峰"
L["Paragon chest"] = "巔峰箱"
L["Paste your profile string here to import the profile."] = "在這裡貼上設定檔字串來匯入設定檔。"
L["Pet number modifier"] = "寵物數量修正"
L["Pixel Perfect Mode"] = "完美細緻模式"
L["PL"] = "長"
L["Player Buff Growth Direction"] = "玩家增益圖示延伸方向"
L["Player de/buff animation"] = "玩家增益/減益動畫效果"
L["Player Debuffs Growth Direction"] = "玩家減益圖示延伸方向"
L["Player frame in group"] = "隊伍中也要顯示自己"
L["Player frame in target frame style"] = "玩家框架使用目標框架風格"
L["Popup notifications"] = "彈出通知"
L["Prevent the same messages from displaying in chat more than once within this set amount of seconds, set to zero to disable."] = "避免在設定的秒數內多次顯示相同的訊息，設為 0 會停用此功能。"
L["Preview Raid Frames"] = "預覽團隊框架"
L["Profile string to share your settings:"] = "分享設定用的設定檔字串:"
L["PROFILES"] = "設定檔"
L["Profiles"] = "設定檔"
L["Profiles are an easy way to share your settings across characters and realms."] = "設定檔可以讓多個角色和不同伺服器共用相同設定，是最簡單的方式。"
L["Profit:"] = "收益:"
L["PvP Indicator"] = "PvP 狀態圖示"
L["QA Testing by"] = "品管測試"
L["Quick Join Messages"] = "快速加入訊息"
L["R"] = "團"
L["Raid Auras"] = "團隊光環"
L["Raid Indicators"] = "團隊光環圖示"
L["Raid-Wide Sorting"] = "排序團隊"
L["Rare on minimap"] = "小地圖上的稀有怪"
L["Ready to craft"] = "準備好可以製造"
L["Red Overlay"] = "覆蓋紅色"
L["Rename layout:"] = "重新命名版面配置:"
L["Repair needed"] = "需要修裝"
L["Replace the default character window."] = "取代預設的角色資訊視窗。"
L["Replace the default fonts withGw2 UI fonts."] = "使用 GW2 UI 字型。"
L["Replace the default mana/power bar."] = "取代法力/能量條。"
L["Replace the default UI chat bubbles. (Only in not protected areas)"] = "取代預設的聊天泡泡。(只在允許更改聊天泡泡的區域/副本外面)"
L["Replace the default UI group frames."] = "取代預設的團隊和小隊框架。"
L["Replace the default UI tooltips."] = "取代預設的浮動提示資訊。"
L["Required Items:"] = "需要物品:"
L["Researchers"] = true
L["Reset Character Data: Hold Shift + Right Click"] = "重置角色資料: 按住 Shift + 右鍵點擊"
L["Reset Session Data: Hold Ctrl + Right Click"] = "重置本次登入資料: 按住 Ctrl + 右鍵點擊"
L["Rested "] = "休息加成"
L["Retrieve your corpse"] = "撿屍體"
L["Reverse Bag Order"] = "反向背包順序"
L["Right"] = "右側"
L["Right Bar Width"] = "右方快捷列寬度"
L["Right Click to change Talent Specialization"] = "點右鍵更改天賦專精"
L["Right click to customize the bag title."] = "點一下右鍵自訂背包標題"
L["Ritual of Summoning"] = "召喚儀式"
L["RL"] = "長"
L["Role Bar"] = "角色職責列"
L["Role Icon"] = "角色職責圖示"
L["RW"] = "警"
L["Save and Reload"] = "儲存並重新載入"
L["Saved Dungeon(s)"] = "已保存的地城"
L["Saved Raid(s)"] = "已保存的團隊"
L["Scale"] = "縮放大小"
L["Scale with Right Click"] = "點一下右鍵來縮放大小"
L["Scales the UI into a Pixel Perfect Mode. This is dependent on screen resolution."] = "介面縮放成完全符合像素的模式，會依據畫面的解析度而定。"
L["Scroll Interval"] = "捲動行數"
L["Scroll Messages"] = "捲動訊息"
L["Secure"] = "安全"
L["Sell junk automatically"] = "自動賣垃圾"
L["Selling junk"] = "賣垃圾"
L["Separate bags"] = "分離背包"
L["Seperate"] = "分開"
L["Session:"] = "本次登入:"
L["Set dispellable debuff scale"] = "設定可驅散減益的縮放大小"
L["Set important Dungeon & Raid debuff scale"] = "設定重要的地城 & 團隊減益的縮放大小"
L["Set Raid Anchor"] = "設定團隊對齊點"
L["Set Raid Frame Container Height"] = "團隊框架整體高度"
L["Set Raid Frame Container Width"] = "團隊框架整體寬度"
L["Set Raid Growth Direction"] = "設定團隊延伸方向"
L["Set Raid Unit Height"] = "團隊單位高度"
L["Set Raid Unit Width"] = "團隊單位高度"
L["Set Raid Units per Column"] = "設定團隊每行數量"
L["Set the empty action bar slots alpha value."] = "設定空的快捷列欄位透明度。"
L["Set the grow direction for raid frames."] = "設定團隊框架增長延伸的方向。"
L["Set the growth direction of the stance bar"] = "設定姿勢形態列展開時的方向。"
L["Set the height of the raid units."] = "設定一個團隊單位的高度。"
L[ [=[Set the maximum height that the raid frames can be displayed.

This will cause unit frames to shrink or move to the next column.]=] ] = [=[設定整個團隊框架可以顯示的最大高度。

單位框架會隨之縮小或移動到下一行。]=]
L["Set the maximum number of lines displayed in the Copy Chat Frame"] = "設定複製聊天視窗最多顯示幾行。"
L[ [=[Set the maximum width that the raid frames can be displayed.

This will cause unit frames to shrink or move to the next row.]=] ] = [=[設定整個團隊框架可以顯示的最大寬度。

單位框架會隨之縮小或移動到下一列。]=]
L["Set the number of raid unit frames per column or row, depending on grow directions."] = "依據延伸方向，設定每一行或每一列的團隊框架單位數量。"
L["Set the order that the group will sort."] = "設定隊伍的排列順序。"
L["Set the width of the raid units."] = "設定一個團隊單位的寬度。"
L[ [=[Set where the raid frame container should be anchored.

By position: Always the same as the container's position on screen.
By growth: Always opposite to the growth direction.]=] ] = [=[設定整個團隊框架要如何對齊。

依據位置: 永遠和整個框架在畫面上的位置相同。
依據延伸: 永遠和框架延伸的方向相反。]=]
L["Settings are not available in combat!"] = "戰鬥中無法調整設定!"
L["Setup"] = "設定"
L["Setup Chat"] = "設定聊天視窗"
L["Setup CVars"] = "設定 CVars 遊戲預設參數"
L["Shift + Left Click"] = "Shift+左鍵"
L["Shift Click: Collect Garbage"] = "Shift+左鍵: 收集垃圾"
L["Short Channels"] = "縮寫頻道名稱"
L["Shorten the channel names in chat."] = "聊天中的頻道名稱顯示成縮寫。"
L["Show a separate grid for raid pets"] = "用分開的團隊框架顯示寵物"
L["Show alternative background texture"] = "顯示另一種背景材質"
L["Show an additional resource bar"] = "顯示額外資訊條"
L["Show Aura Tooltips"] = "顯示光環的浮動提示資訊"
L["Show auras below"] = "在下方顯示光環"
L["Show both party frames and party grid"] = "同時顯示隊伍和團隊框架"
L["Show character item info"] = "顯示角色物品資訊"
L["Show classpower bar only in combat"] = "只有戰鬥中才顯示職業能量條"
L["Show clickable Quick Join messages inside of the chat."] = "在聊天視窗中顯示可點擊的快速加入通知訊息。"
L["Show Combo Points on Target"] = "在目標框架顯示連擊點數"
L["Show combo points on target, below the health bar."] = "在目標框架的血條下方顯示連擊點數。"
L["Show Coordinates on Minimap"] = "小地圖顯示坐標"
L["Show Coordinates on World Map"] = "世界地圖顯示坐標"
L["Show Country Flag"] = "顯示國旗"
L["Show FPS on minimap"] = "小地圖上顯示 FPS"
L["Show grid"] = "顯示格子"
L["Show healing numbers"] = "顯示治療數字"
L["Show health as a numerical value."] = "顯示血量數值。"
L["Show Health bar text"] = "顯示血量條文字"
L["Show HUD background"] = "顯示 HUD 背景"
L["Show important Dungeon & Raid debuffs"] = "顯示重要的地城 & 團隊減益效果。"
L["Show Junk Icon"] = "顯示垃圾圖示"
L["Show Macro Name"] = "顯示巨集名稱"
L["Show Macro Name on Action Button"] = "在快捷列上顯示巨集名稱"
L["Show Missing Raid Buffs Bar"] = "顯示缺少的團隊增益列"
L["Show or hide auras and edit raid aura indicators."] = "顯示或隱藏光環，以及編輯增益/減益圖示。"
L["Show premade group info"] = "顯示預組隊伍資訊"
L["Show Profession Bag Coloring"] = "專業背包著色"
L["Show Quality Color"] = "顯示品質顏色"
L["Show Remaining Time"] = "顯示剩餘時間"
L["Show Role Bar"] = "顯示角色職責列"
L["Show Scrap Icon"] = "顯示 Scrap 圖示"
L["Show Shield Value"] = "顯示護盾值"
L["Show Spell Icons"] = "顯示法術圖示"
L["Show spell icons instead of monochrome squares."] = "顯示法術圖示，而不是單色的方塊。"
L["Show spell queue window on castingbar"] = "在施法條上顯示法術佇列視窗"
L["Show the remaining aura time as an animated overlay."] = "用動畫效果顯示光環的剩餘時間。"
L["Show Threat"] = "顯示仇恨值"
L["Show tooltips of buffs and debuffs."] = "顯示增益和減益效果的浮動提示資訊。"
L["Show Upgrade Icon"] = "顯示升級圖示"
L["Show your player frame as part of the group"] = "在隊伍框架中顯示玩家自己。"
L["Shows an animation for new de/buffs"] = "新的增益/減益顯示動畫效果"
L["Shows the xp you got from that quest in % based on your total needed xp for next level."] = "顯示該任務所獲得的經驗值，是升級所需經驗值的多少 %。"
L["Siege On Dragonbane Keep"] = "進攻龍禍要塞"
L["Size and Positions"] = "大小和位置"
L["Skins"] = "美化外觀"
L["Skip"] = "繼續"
L["Socket Frame"] = "珠寶插槽"
L["Sort Direction"] = "排序方向"
L["Sort Method"] = "排序方式"
L["Sort Raid Frames by Role"] = "依角色職責排序團隊框架"
L["Sort raid unit frames by role (tank, heal, damage) instead of group."] = "依據角色職責排序團隊框架 (坦克、治療者、傷害輸出) 而不是隊伍。"
L["Sort to Last Bag"] = "排序到最後的背包"
L["Soulwell"] = "靈魂之井"
L["Spam Interval"] = "洗頻間隔時間"
L["Spec is already assigned to a layout!"] = "專精已經分配過版面配置!"
L["Spent:"] = "花費:"
L["Stacking"] = "層數"
L["Stance Bar Growth Direction"] = "姿勢形態列展開方向"
L["Start installation"] = "開始安裝"
L["Start Near Center"] = "從中間開始"
L["Stop alert when the event is completed in this week."] = "本週已完成的任務不要提醒。"
L["System info"] = "系統資訊"
L["Talking Head"] = "說話的頭框架"
L["Target Info"] = "目標資訊"
L["Targeted by:"] = "被關注:"
L["Text has not loaded."] = "看到這個訊息時表示我們忘記載入一些文字。不過不用擔心，會有一些適合的範例文字來告訴您相關資訊，就像現在這樣。"
L["The following addon(s) are loaded, which can cause conflicts. By default, this setting is disabled."] = "下列已載入的插件可能會造成衝突，預設停用此功能模組。"
L["The Group Manage Button will fade when you move the cursor away."] = "淡出隊伍管理按鈕，滑鼠游標靠近時才會顯示。"
L["The HUD background changes color in the following situations: In Combat, Not In Combat, In Water, Low HP, Ghost"] = "在這些情況下，主要快捷列後方的 HUD 背景會隨之變化: 戰鬥中、非戰鬥中、在水中、低血量、鬼魂狀態。"
L["The initial group will start near the center and grow out."] = "隊伍會從中間開始向外延伸。"
L["The main menu icons will fade when you move your cursor away."] = "隱藏畫面左上角的微型選單，滑鼠游標靠近時才顯示。"
L["The number of pages shown in the merchant frame. Set 1 to disable."] = "商人視窗所顯示的頁數，設為 1 會停用。"
L["This part sets up your chat window names, positions, and colors."] = "這部分會設定聊天視窗名稱、位置和顏色。"
L["This part sets up your World of Warcraft default options."] = "這部分會設定魔獸世界選項的預設值。"
L["This short installation process will help you to set up all of the necessary settings used by GW2 UI."] = "這個簡短的安裝程序，會幫你設定好使用 GW2 UI 所有需要的設定，"
L["Time"] = "時間"
L["Time Rift"] = true
L["Toggle Compass"] = "顯示羅盤"
L["Toggle the borders around the screen"] = "在畫面周圍顯示邊框"
L["Tooltips"] = "浮動提示資訊"
L["Top"] = "上方"
L["Top Left"] = "左上"
L["Top Right"] = "右上"
L["Total CPU:"] = "CPU 總用量:"
L["Trading post"] = "貿易站"
L["Turn Pixel Perfect Mode On"] = "開啟完美細緻模式"
L["Up"] = "上"
L["Up and right"] = "右上"
L["Upcoming Level Rewards"] = "即將獲得的升級獎勵"
L["URL Links"] = "URL 連結"
L["Use class color for the names of players when they are mentioned."] = "提到某位玩家時，用職業顏色顯示他的名字。"
L["Use the class color instead of class icons."] = "使用職業顏色而不是職業圖示。"
L["Use the GW2 UI improved action bars."] = "使用 GW2 UI 風格加強型的快捷列。"
L["Use the GW2 UI improved Pet bar."] = "使用 GW2 UI 風格加強型的寵物列。"
L["Use the GW2 UI Minimap frame."] = "使用 GW2 UI 小地圖框架。"
L["Used for animations"] = "供動畫使用"
L["Vertical"] = "垂直"
L["Vertical Spacing"] = "垂直間距"
L["Vignettes"] = "小地圖星號"
L["Watch list"] = "監控清單"
L["Weekly Reset"] = "每週重置"
L["Welcome"] = "歡迎使用"
L["Welcome to GW2 UI"] = "歡迎使用 GW2 UI"
L[ [=[What is 'Pixel Perfect'?

GW2 UI has a built-in setting called 'Pixel Perfect Mode'. What this means for you is that your user interface will look as was intended, with crisper textures and better scaling. Of course, you can toggle this off in the settings menu should you prefer.]=] ] = [=[什麼是 '完美細緻'?

GW2 UI 內建了一項設定叫做 '完美細緻模式'，擁有更清晰的材質和更佳的縮放大小處理，讓每一個像素都趨近完美，使用者介面看起來能夠完全符合你的期望。當然，完全可以依據你的喜好來開關這個設定。]=]
L["When in a raid group, show if anyone in your raid is targeting the current tooltip unit."] = "在團隊中時，顯示將目前浮動提示資訊單位選取為目標的人。"
L["When you go AFK, display the AFK screen."] = "AFK 時顯示暫離畫面。"
L["Whether to display a floating bar showing your group or raid's role composition. This can be moved via the 'Move HUD' interface."] = "是否要顯示隊伍或團隊的角色組成列，可透過 '移動 HUD' 來調整位置。"
L["Whether to display a floating bar showing your missing buffs. This can be moved via the 'Move HUD' interface."] = "是否要顯示你缺少的增益效果列，可透過 '移動 HUD' 來調整位置。"
L["Width"] = "寬度"
L["With Contributions by"] = "貢獻"
L["World Latency:"] = "世界延遲:"
L["World Protocol:"] = "世界協定:"
L["XP Quest Percent"] = "經驗值任務百分比"
L["You can not move elements during combat!"] = "戰鬥中無法移動介面!"
L["You have %s pending calendar invite(s)."] = "你有 %s 個待回覆的行事曆活動邀請。"
L["You have %s pending guild event(s)."] = "你有 %s 個待回覆的公會活動邀請。"
L["You have now finished installing GW2 UI!"] = "GW2 UI 現在已經安裝完成了!"
L["You have overridden this behavior."] = "你已選擇使用其他插件取代此功能。"
L["Your Auras First"] = "你的光環優先"
L["Your items have been repaired for: %s"] = "已修理裝備，共支出: %s"
L["Your items have been repaired using guild bank funds for: %s"] = "已使用公會資金修理裝備，共支出: %s"
L["Zone Ability"] = "區域技能"