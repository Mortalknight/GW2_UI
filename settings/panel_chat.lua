...
local function LoadChatPanel(sWindow)
    ...
    addOption(p.scroll.scrollchild, L["URL Links"], L["Attempt to create URL links inside the chat."], { getterSetter = "CHAT_FIND_URL", callback = GW.UpdateChatSettings, dependence = {["CHATFRAME_ENABLED"] = true}})
    addOption(p.scroll.scrollchild, L["Hyperlink Hover"], L["Display the hyperlink tooltip while hovering over a hyperlink."], { getterSetter = "CHAT_HYPERLINK_TOOLTIP", callback = GW.UpdateChatSettings, dependence = {["CHATFRAME_ENABLED"] = true}})
    addOption(p.scroll.scrollchild, L["Short Channels"], L["Shorten the channel names in chat."], { getterSetter = "CHAT_SHORT_CHANNEL_NAMES", callback = GW.UpdateChatSettings, dependence = {["CHATFRAME_ENABLED"] = true}})
    ...
    addOption(p.scroll.scrollchild, L["Emotion Icons"], L["Display emotion icons in chat"], {getterSetter = "CHAT_KEYWORDS_EMOJI", callback = function(value) GW.UpdateChatSettings() GW_EmoteFrame:Hide() for _, frameName in ipairs(CHAT_FRAMES) do if _G[frameName].buttonEmote then _G[frameName].buttonEmote:SetShown(value) end end end, dependence = {["CHATFRAME_ENABLED"] = true}})
    ...
    addOptionSlider(p.scroll.scrollchild, L["History Size"], nil, { getterSetter = "historySize", min = 10, max = 500, decimalNumbers = 0, step = 1, dependence = {["CHATFRAME_ENABLED"] = true, ["chatHistory"] = true}})
    addOptionButton(p.scroll.scrollchild, L["Reset History"], nil, {callback = function() GW.private.ChatHistoryLog = {} end})

    addOptionDropdown(p.scroll.scrollchild, TIMESTAMPS_LABEL, OPTION_TOOLTIP_TIMESTAMPS, { getterSetter = "timeStampFormat", optionsList = {"NONE", "%I:%M ", "%I:%M:%S ", "%I:%M %p ", "%I:%M:%S %p ", "%H:%M ", "%H:%M:%S "}, optionNames = {NONE, "03:27", "03:27:32", "03:27 PM", "03:27:32 PM", "15:27", "15:27:32"}})
    addOptionDropdown(p.scroll.scrollchild, L["Announce Interrupts"], L["Announce when you interrupt a spell to the specified chat channel"], { getterSetter = "interruptAnnounce", callback = GW.ToggleInterruptAnncouncement, optionsList = {"NONE", "SAY", "YELL", "PARTY", "RAID", "RAID_ONLY", "EMOTE"}, optionNames = {NONE, SAY, YELL, L["Party Only"], L["Party / Raid"], L["Raid Only"], EMOTE}})

    local soundKeys = {}
    for _, sound in next, GW.Libs.LSM:List("sound") do
        tinsert(soundKeys, sound)
    end
    addOptionDropdown(p.scroll.scrollchild, L["Keyword Alert"], nil, { getterSetter = "CHAT_KEYWORDS_ALERT_NEW", callback = GW.UpdateChatSettings, optionsList = soundKeys, optionNames = soundKeys, dependence = {["CHATFRAME_ENABLED"] = true}, hasSound = true})
    ...
    addOptionSlider(p.scroll.scrollchild, L["Scroll Messages"], L["Number of messages you scroll for each step."], { getterSetter = "CHAT_NUM_SCROLL_MESSAGES", callback = function() GW.UpdateChatSettings() end, min = 1, max = 12, decimalNumbers = 0, step = 1, dependence = {["CHATFRAME_ENABLED"] = true}})
    ...
    addOptionSlider(p.scroll.scrollchild,  L["Maximum lines of 'Copy Chat Frame'"],  L["Set the maximum number of lines displayed in the Copy Chat Frame"], { getterSetter = "CHAT_MAX_COPY_CHAT_LINES", min = 50, max = 500, decimalNumbers = 0, step = 1, dependence = {["CHATFRAME_ENABLED"] = true}})
