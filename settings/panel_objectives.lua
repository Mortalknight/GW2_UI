...
local function LoadObjectivesPanel(sWindow)
    ...
    addOption(p.scroll.scrollchild, L["Show Objective Tracker progress bars"], L["If disabled, progress bars will not be shown for various objective tracker items such as quests, achievements, etc."], "QUESTTRACKER_STATUSBARS_ENABLED", function() GwQuesttrackerContainerQuests:UpdateLayout(); GwQuesttrackerContainerAchievement.UpdateLayout(); GwQuesttrackerContainerBonusObjectives:UpdateLayout(); GW.UpdateCollectionTrackingLayout(GwQuesttrackerContainerCollection); GW.UpdateMonthlyActivitesTracking(GwQuesttrackerContainerMonthlyActivity); GW.UpdateRecipeTrackingLayout(GwQuesttrackerContainerRecipe); GW.updateCurrentScenario(GwQuesttrackerContainerScenario); GW.QuestTrackerLayoutChanged() end, nil, {["QUESTTRACKER_ENABLED"] = true})
