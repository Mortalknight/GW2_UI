...
local function LoadModulesPanel(sWindow)
    ...
    addOption(p.scroll.scrollchild, DISPLAY_POWER_BARS, L["Replace the default mana/power bar."], "POWERBAR_ENABLED", function() GwPlayerPowerBar:ToggleBar(); GW.UpdateClassPowerExtraManabar() end)
    ...
    addOption(p.scroll.scrollchild, TRADE_SKILLS, L["Enable the profession replacement."], "USE_PROFESSION_WINDOW", function() GW.ShowRlPopup = true end)
    ...
    addOption(p.scroll.scrollchild, L["Alert Frames"], nil, "ALERTFRAME_ENABLED", function() GW.ShowRlPopup = true end)
