...
local function LoadTooltipPanel(sWindow)
    ...
    p:AddOption(L["Show Health bar text"], nil, {getterSetter = "ADVANCED_TOOLTIP_SHOW_HEALTHBAR_TEXT", callback = function(value) if not GameTooltip:IsForbidden() then if value then GameTooltipStatusBar.Text:Show() else GameTooltipStatusBar.Text:Hide() end end end, dependence = {["TOOLTIPS_ENABLED"] = true}})
