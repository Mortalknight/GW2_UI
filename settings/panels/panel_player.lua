...
local function LoadPlayerPanel(sWindow)
    ...
    p_player_aura:AddOption(GW.NewSign .. L["Pandemic Highlight"], L["Highlights your own auras while they are inside their refresh window, where refreshing adds the remaining time on top."], {getterSetter = "PLAYER_PANDEMIC_HIGHLIGHT", callback = GW.UpdateAuraOptionHolders, dependence = {["PLAYER_BUFFS_ENABLED"] = true}, hidden = not GW.Retail})
    p_player_aura:AddOption(GW.NewSign .. L["Show Dispel Type Icon"], L["Shows the dispel type of auras as a small icon in the corner of the aura."], {getterSetter = "PLAYER_DISPEL_ICON", callback = GW.UpdateAuraOptionHolders, dependence = {["PLAYER_BUFFS_ENABLED"] = true}, hidden = not GW.Retail})
